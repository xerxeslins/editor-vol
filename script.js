...
function addBlock(type) {
    ...
    if (type === 'comando') {
        ...
        placeholder = "Cole seu código fonte aqui...";
...
// --- LIMPEZA DE HTML (CORRIGIDO PARA ESPAÇAMENTO) ---
function cleanContent(html) {
    ...
    // CORREÇÃO: Trata linhas vazias (<div><br></div>) como apenas uma quebra
    // Isso evita o triplo BR.
    clean = clean.replace(/<div[^>]*>\s*<br\s*\/?>\s*<\/div>/gi, '<br>');

    // Normaliza outras DIVs para BRs
    clean = clean.replace(/<div[^>]*>/gi, '<br>');
    clean = clean.replace(/<\/div>/gi, '');

    // TRAVA FINAL: Substitui qualquer sequência de 3 ou mais BRs por apenas 2
